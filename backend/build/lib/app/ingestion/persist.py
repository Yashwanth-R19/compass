...
# Session 07 (Risk v2, Part D.2): a graded successor to the coarse `is_fix`
# regex (app/ingestion/miner.py::FIX_RE), which matches "fix typo in
# README" exactly as confidently as a real bug fix. STRONG_FIX_RE covers the
# words that are rarely anything BUT a fix; WEAK_FIX_RE covers "close(s/d)",
# which commonly closes a feature PR/issue, not a bug -- see
# compute_fix_confidence's docstring for the exact scoring.
_STRONG_FIX_RE = re.compile(r"\b(fix|bug|patch|resolve)\b", re.IGNORECASE)
_WEAK_FIX_RE = re.compile(r"\bclose[sd]?\b", re.IGNORECASE)
_ISSUE_REF_RE = re.compile(r"#\d+")


def compute_fix_confidence(message: str) -> float:
    """Session 07 (Risk v2, Part D.2): 1.0 when the message references an
    issue number (``#123``) AND matches the fix pattern (strong or weak);
    0.7 when it matches the fix pattern alone (a strong word, no issue
    ref); 0.3 when the ONLY match is a weak word like "close"/"closes"
    (which commonly means "closes #123 [a feature]", not a bug fix); 0.0
    otherwise. Independent of, and does not replace, `is_fix` -- kept as-is
    for backward compatibility (CLAUDE.md)."""
    has_issue_ref = bool(_ISSUE_REF_RE.search(message))
    has_strong = bool(_STRONG_FIX_RE.search(message))
    has_weak = bool(_WEAK_FIX_RE.search(message))
    ...
    commit_rows = [
        {
            ...
            "fix_confidence": compute_fix_confidence(c.message),
    ...
    file_rows = [
        {
            ...
            "churn_recent_365d": f.churn_recent_365d,
            "churn_weighted": f.churn_weighted,
        }
        for f in mined.files
    ]
    if file_rows:
