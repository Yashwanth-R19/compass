...
class HygieneEngine(Engine):
    ...
    def run(self, ctx: RunContext, session: Session) -> dict[str, Any]:
        ...
        if path_ids:
            ...
            # Targets the raw Table (FileMetrics.__table__), not the mapped
            # class -- a plain SQLAlchemy Core bulk UPDATE via executemany,
            # matching persist.py's "Core insert() with lists of dicts, not
            # ORM object-per-row" discipline. update(FileMetrics) (the
            # mapped class) makes this an "ORM-enabled" UPDATE instead,
            # which tries to synchronize in-memory ORM object state and
            # rejects a bulk WHERE-keyed-by-non-primary-key executemany --
            # this engine never loads FileMetrics as ORM objects, so there
            # is nothing to synchronize and the Core path is the correct,
            # simpler one.
            session.execute(
