...
"""A genuine spec/example conflict, resolved in favor of the required test
fixture -- same precedent as app/analysis/identities.py::MIN_LOCAL_PART_LENGTH
(session 05). The prose says "ratio < 0.2"; the session's own required
fixture (source changes 10 times, its test changes in 2 of those ->
ratio == 0.2 exactly) requires that boundary value to classify
"stale_test". Only a `<=` comparison satisfies both the literal fixture and
the general spirit ("meaningfully low co-change"), so the comparison below
is `<=`, not `<`."""
...
class TestGapEngine(Engine):
    ...
    def run(self, ctx: RunContext, session: Session) -> dict[str, Any]:
        ...
        if result_by_path:
            ...
            # See app/engines/hygiene.py's identical bulk-UPDATE for why this
            # targets the raw Table (FileMetrics.__table__) rather than the
            # mapped class -- a plain Core bulk UPDATE, not an ORM-enabled
            # one, since this engine never loads FileMetrics as ORM objects.
            session.execute(
                update(FileMetrics.__table__)  # type: ignore[arg-type]
                .where(
                    FileMetrics.__table__.c.analysis_run_id == run_id,
                    FileMetrics.__table__.c.path_id == bindparam("b_path_id"),
                )
                .values(
