...
// the SAME 12-colour categorical palette the subsystem graph/treemap/city
// use (an ordered slice, not hashed -- shares are already sorted by size,
// so position IS the ranking).
...
/** Merges the former Passport and Health pages (UI rebuild session 3, Part
 * A) -- the landing surface for every repository. Every section below
 * gates on the SAME "onboarding" stage via the one `usePassport` call
 * (table 4.4: "overview | passport, difficulty, health | onboarding") --
 * `PassportEngine` runs last in that stage and embeds `HealthEngine`'s own
 * just-written row directly into `data.health`, so there is no separate
 * `/health` fetch needed here at all. The run-history sparkline is the one
 * genuine exception: it describes PAST runs, not this run's own gate, so
 * it renders independently and never blocks on the passport StageGate. */
...
function IdentityStrip({
  ...
  return (
    <Card
      ...
      action={
        <Link
          to={`/repos/${repo.id}/map?view=city`}
...
function ThreeThingsCard({ items, repoId }: { items: PassportFirstPrItem[]; repoId: string }) {
  return (
    <Card title="Three things to know">
      ...
      {items.length === 0 ? (
        <p className="text-sm text-text-muted">Nothing stood out sharply enough to flag here.</p>
      ) : (
...
function Stat({ label, value }: { label: string; value: number }) {
  return (
    <div>
      ...
      <dd className="cp-stat text-lg font-semibold text-text">{value.toLocaleString()}</dd>
    </div>
  );
}
    ...
    >
      <div className="flex flex-col gap-4">
        <div className="flex items-center gap-6">
          <div>
            <p className="cp-stat text-3xl font-semibold text-text">{team.truck_factor}</p>
