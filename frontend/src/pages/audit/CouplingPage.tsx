...
export function CouplingPage() {
  ...
  const hiddenData = hiddenDeps.data?.kind === "data" ? hiddenDeps.data.data : undefined;

  const hiddenPairKeys = useMemo(() => {
    if (!hiddenData) return new Set<string>();
    return new Set(hiddenData.pairs.map((p) => [p.file_a_path, p.file_b_path].sort().join("|")));
  }, [hiddenData]);

  const { nodes, edges } = useMemo(() => {
    if (!couplingData) return { nodes: [] as CappableNode[], edges: [] as CouplingEdge[] };
    const nodeIds = new Set<string>();
    const builtEdges: CouplingEdge[] = couplingData.pairs.map((p) => {
  ...
  return (
    <StageGate
      query={coupling}
      loadingLabel="Loading coupling data…"
      emptyTitle="No coupling pairs found"
      emptyMessage="Either this repo doesn't have enough shared-revision history yet, or no two files consistently change together."
      ...
      {(data) => (
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_320px]">
            ...
            <Card title="Ranked pairs" subtitle={`${data.pairs.length} total`}>
              <ul className="max-h-[480px] divide-y divide-slate-100 overflow-y-auto text-sm dark:divide-slate-800">
                {data.pairs.map((p) => {
                  const isHidden = hiddenPairKeys.has(
                    [p.file_a_path, p.file_b_path].sort().join("|"),
                  );
                  return (
